...
# Option
def parse_args():
    parser = argparse.ArgumentParser(description='MMMF-Over-GS')
...
def train(dataset, hyper_dict, multi_class_flip, patience, classification_save_root_path):
    ...
    for cv in range(5):
        ...
        mf_common_optimizer = torch.optim.Adam(common_param, lr=hyper_dict['clf_lr'],
                                               weight_decay=hyper_dict['clf_reg'])
        ...
        for i in range(hyper_dict['epoch']):
            ...
            if i > patience:
                # Reconstruction CLF Grad of U
                u_not_over = torch.tensor(net.u[train_index].cpu().detach().numpy(), requires_grad=True).float()
                u_not_over.retain_grad()

                # SoftTriple
                soft_loss = clf(u_not_over.to(hyper_dict['device']), torch.tensor(y_train).to(hyper_dict['device'])).to(
                    hyper_dict['device'])
                soft_loss.backward()

                g4 = torch.zeros_like(net.u.grad).to(hyper_dict['device'])
                g4[train_index] = u_not_over.grad[:len(train_index)].clone().to(hyper_dict['device'])
                net.u.grad.zero_()

                # Mean Grad => U[train_index] -> g1, g2, g3, g4, U[~train_index] -> g1, g2, g3
                mean_grad = (g1 + g2 + g3 + g4) / 4
    ...
    result = {'Validation': {'BA': validation_ba_list, 'F1': validation_f1_list, 'AUC': validation_auc_list,
                             'MCC': validation_mcc_list},
...
if __name__ == '__main__':
    ...
    # Print Argument
    print('MMMF-Over-GS // Data Type: {} MP: {}'.format(data_type, mp))

    # Save Path
    performance_save_root_path = os.path.join('./Result/Performance/', 'Patience_' + str(args.re_patience), str(mp), data_type, 'MMMF-Over-GS')
    createFolder(performance_save_root_path)

    classification_save_root_path = os.path.join('./Result/Classifcation_Model/', 'Patience_' + str(args.re_patience), str(mp), data_type, 'MMMF-Over-GS')
    for cv in range(5):
    ...
    # Grid Search
    for all_hyper_parameter in tqdm(hyper_parameters_list, desc='MMMF-Over-GS Hyperparameter Search...'):
