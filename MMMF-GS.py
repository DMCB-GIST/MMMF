...
# Option
def parse_args():
    parser = argparse.ArgumentParser(description='MMMF-GS')
    ...
    for cv in range(5):
        ...
        for i in range(hyper_dict['epoch']):
            ...
            if i > patience:
                ...
                # SoftTriple
                soft_loss = clf(u_over.to(hyper_dict['device']), torch.tensor(y_train_over).to(hyper_dict['device'])).to(hyper_dict['device'])
                soft_loss.backward()
...
if __name__ == '__main__':
    ...
    # Print Argument
    print('MMMF-GS // Data Type: {} MP: {}'.format(data_type, mp))

    # Save Path
    performance_save_root_path = os.path.join('./Result/Performance/', 'Patience_' + str(args.re_patience), str(mp), data_type, 'MMMF-GS')
    createFolder(performance_save_root_path)

    classification_save_root_path = os.path.join('./Result/Classifcation_Model/', 'Patience_' + str(args.re_patience), str(mp), data_type, 'MMMF-GS')
    for cv in range(5):
    ...
    # Grid Search
    for all_hyper_parameter in tqdm(hyper_parameters_list, desc='MMMF-GS Hyperparameter Search...'):
